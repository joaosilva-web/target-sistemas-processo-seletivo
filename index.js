...
document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("resultado").innerText =
    "Portanto o resultado é: " + firstQuestion();
});
...
function filterData(dataReceived) {
  //Function to remove All useless days
...
function getPercentageEachState(data, totalSum) {
  ...
  return arrPercentageEachState;
}
fetch("./faturamentosDistribuidora.json")
  .then((response) => response.json())
  .then((data) => {
    const monthlySum = getMonthlySum(data);
    const percentageEachState = getPercentageEachState(data, monthlySum);
    console.log(percentageEachState);
    document.getElementById(
      "percentageOfStates"
    ).innerHTML = `
    <ul>
